...
def test_generate_normal_distribution_logic():
    ...
    df = DatasetGenerator.generate_normal_distribution(size=1000, mean=5, std=2)
...
def test_generate_random_integers_logic():
    ...
    df = DatasetGenerator.generate_random_integers(size=500, low=10, high=20)
...
def test_generate_uniform_distribution_logic():
    ...
    df = DatasetGenerator.generate_uniform_distribution(size=500, low=0, high=10)
    assert len(df) == 500, "The generated dataset should have 500 rows."
